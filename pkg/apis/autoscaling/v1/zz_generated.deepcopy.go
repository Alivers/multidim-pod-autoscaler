...
func (in *RecommendedResources) DeepCopyInto(out *RecommendedResources) {
	*out = *in
	...
	if in.LowerBoundResource != nil {
		in, out := &in.LowerBoundResource, &out.LowerBoundResource
		*out = make(corev1.ResourceList, len(*in))
		for key, val := range *in {
			(*out)[key] = val.DeepCopy()
		}
	}
	if in.UpperBoundResource != nil {
		in, out := &in.UpperBoundResource, &out.UpperBoundResource
		*out = make(corev1.ResourceList, len(*in))
		for key, val := range *in {
			(*out)[key] = val.DeepCopy()
		}
	}
	if in.UncappedTargetResource != nil {
		in, out := &in.UncappedTargetResource, &out.UncappedTargetResource
		*out = make(corev1.ResourceList, len(*in))
		for key, val := range *in {
			(*out)[key] = val.DeepCopy()
		}
